...
def main(field,collection,solrUrl):
    stvc = SolrTermVectorCollector(field='Body',feature='tf',batchSize=1000,numDocs=1000)
    tdc = TermDocCollection(source=stvc)
    ptdc = PseudoTermDocCollection(termDocCollection=tdc)
    SolrTermVectorSubmittor(field='BodyBlurred')
